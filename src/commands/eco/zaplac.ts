...
export default function zaplac(args, message) {
    ...
    if(typeof komu == 'undefined' || typeof ilosc == 'undefined' || komu.id == message.author.id) return message.channel.send(usageEmbed("*zaplac <ping komu> <ile>"))
    if(ilosc < 0) {
                ...
                if(tuzyt && kuzyt) {
                    if(tuzyt.length && kuzyt.length) {
                        if(tuzyt[0].hajs >= Number(ilosc)) {
                            ...
                            tuzyt[0].save();
                            kuzyt[0].save();
                            const wychodzacyEmbed = new MessageEmbed()
                            ...
                            message.channel.send(wychodzacyEmbed);

                            const przychodzacyEmbed = new MessageEmbed()
                                .setAuthor('Nougat Pay \™', 'https://cdn.discordapp.com/avatars/429587398511427584/a8d77ae510e68cc595c1ccda04a755fa.jpg?size=1024')
                                .setColor((Math.random() * 0xFFFFFF << 0).toString(16))
                                .setTitle('Gratulacje użytkowniku')
