    ...
    if(message.guild.members.cache.get(message.author.id).hasPermission("ADMINISTRATOR")) {
        ...
        if(ranga.includes("usun")) {
            ...
            Nougat.Serwer.find({id: message.guild.id}, (err, dok) => {
                ...
                dok[0].save();
                const ctsEmbed = new MessageEmbed()
                    ...
                    message.channel.send(ctsEmbed)
            })
        } else if(ranga.length > 0) {
            // USTAWIANIE AUTOROLE
            if(message.guild.members.cache.get(client.user.id).hasPermission("MANAGE_ROLES")) {
                if(message.guild.roles.cache.find(role => role.name === ranga)) {
                    Nougat.Serwer.find({ id: message.guild.id }, (err, dok) => {
                        if(err) return;
                        dok[0].autorola = message.guild.roles.cache.find(role => role.name === ranga).id;
                        dok[0].save();
                        const ctsEmbed = new MessageEmbed()
                            .setAuthor('Nougat - Autorole', 'https://cdn.discordapp.com/avatars/429587398511427584/a8d77ae510e68cc595c1ccda04a755fa.jpg?size=1024')
                            .setColor((Math.random() * 0xFFFFFF << 0).toString(16))
                    ...
                    // Nie znaleziono roli
                    message.channel.send(errorEmbed("Nie znaleziono takiej roli"))
                }
            } else {
                // Nie ma uprawnienia
                message.channel.send(errorEmbed("Nie posiadam odpowiedniego uprawnienia: Zarządzanie rolami"))
            }
        } else {
            // Nie ma argumentów
            const ctsEmbed = new MessageEmbed()
        ...
        message.channel.send(errorEmbed("Nie posiadasz odpowiednich uprawnień"))
    }
}