...
export default function nazwa(args, message, client) {
    ...
    if(message.guild.members.cache.get(message.author.id).hasPermission("MANAGE_NICKNAMES") || message.author.id == config.id) {
        if(message.guild.members.cache.get(client.user.id).hasPermission("CHANGE_NICKNAME")) {
            message.guild.members.cache.get(client.user.id).setNickname(nowanazwa);
